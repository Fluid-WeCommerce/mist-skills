...
> **NEVER reuse data from a previous run.** This skill is used across many different companies. Always start clean with the company being onboarded.
...
Ask for the company website URL before doing anything. Also ask for the optional primary contact. The Fluid store URL and API token are not needed — the active company is already selected in Mist Desktop and `fluid_api` injects the token automatically.
...
2. **Primary contact name** — Who will be the person primarily filling out the onboarding form? If unknown, leave blank — we'll report all people we find and you can assign later.
...
## Step 2: Confirm Active Company

The active Fluid company is already selected in Mist Desktop, so there are no credentials to validate. Do one light confirmation that you're onboarding the right company, plus a reachability check on the website to scrape.
...
[Preflight] Active company: {Company Name} (Company ID: {id})
```

Confirm the active company is the one being onboarded before pushing any data:

```
Active Fluid company: "Yellowbird Foods" (Company ID: 980243068)
Scraping: https://yellowbirdfoods.com

Is this the correct company to onboard? (yes/no)
```
...
```
fluid_api("/api/companies/{company_id}/owners", "POST", {
  "owner": {
    "entity_id": "{entity_id}",
    "full_name": "...",
    "work_email": "...",
...
Fetch `fluid_api("/api/mcc_codes", "GET")` and `fluid_api("/api/business_types?country_code={country}", "GET")`.
