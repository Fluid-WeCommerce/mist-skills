...
- `variant_countries` is an **object keyed by ISO** — `variant_countries.US = { active, price, currency_code, buyable, cv, qv, country_id, display_price }`. It is not an array and has no `country_iso` field. `cv` and `qv` are **retail** volume; the preferred-customer pair is not here (see below).

## Known blind spots — say so, don't imply they passed

Two things live in `variant_countries` in the database and are **deliberately stripped from this endpoint**, so no check in this skill can see them. `Api::V202604::ProductBlueprint` renders variants at `pricing_tier: :retail` and then deletes these keys unconditionally — there is no view, param or tier that returns them:

```
wholesale · wholesale_subscription_price · cost_of_goods_sold · pc_cv · pc_qv
```

**Wholesale pricing is therefore unauditable here**, and so is preferred-customer volume. That matters most for exactly the companies this skill is aimed at: a direct-sales catalog where reps buy at wholesale can have every retail price correct and every wholesale price missing, and this audit will return a clean score. The legacy `/api/company/v1/products` is not a way around it — it renders one pricing tier, the *viewer's* (`viewer_pricing_tier` → `current_pricing_tier`), not both, and the skill cannot choose which.

So when the company runs volume — the same catalog-wide test as *Volume not set*: any non-zero `cv` or `qv` anywhere — add one line under *Informational* saying wholesale prices and preferred-customer volume were **not checked** and why, and that they need a look in the admin. A blind spot named is a blind spot the user can cover; a blind spot silently folded into a 100/100 is a claim the audit did not earn.
...
| Volume not set | `variant_countries[ISO].cv` **or** `qv` is 0/null on a variant, **while at least one other market row in the catalog carries a non-zero value**. That gate is the whole check: a company running no comp plan has zeros everywhere and nothing is wrong, so compute the catalog-wide maximum of `cv` and `qv` first and only run this when it is above zero. Then every zero is a product that earns a rep no volume — commissions, ranks and qualification all read from these fields, so an import that populated volume on some rows and not others is silently paying the wrong amounts. Report `cv` and `qv` as one finding per variant, naming which of the two is missing. Note `pc_cv` / `pc_qv` (preferred-customer volume) are **pruned from this endpoint** and cannot be checked — see *Known blind spots* | variant |
...
| Duplicate title or slug | Two categories, two collections, or a category and a collection sharing a title or slug — usually an importer that created both kinds from the same source taxonomy. Say which pair, since two records with the same title are two entry points into the same range. **State it as a data fact about the records, not about the site** — "a category and a collection both named Adults, same slug", never "the storefront shows two identical doors": you have not looked at the storefront or its navigation, and whether the theme renders both is exactly the kind of claim the scope rule forbids. **Population is affected resources, not pairs:** three duplicated pairs are six affected resources, so `10 × 6/15`, never `10 × 3/15`. Counting pairs halves the deduction and makes the score depend on who ran the audit |
...
- `cv` / `qv` of `0` are normal for a company that doesn't run a comp plan — but **the absolute value is the wrong test**. Never flag a zero on its own; flag the *inconsistency* instead (see *Volume not set* in the product table). If every market row in the catalog is zero, the company does not run volume and there is nothing to report. If some are non-zero, the company does run volume and the zeros are an import gap.
...
- **Utility taxonomy resources are excluded from the art, copy and SEO checks.** Sets that exist to drive theme behaviour rather than browsing — Fluid's own **All Products** default collection, plus `default-suggested-items` / "Recommended Products" and `non-commission-products` style sets — are created by the platform or by a theme, not by a merchant, and nobody is ever going to write tile art, body copy or a meta description for them. Leaving them in the *Missing image* / *Missing description* / *Boilerplate SEO* populations charges every company a permanent deduction it cannot clear: those two checks alone carry 25 of the taxonomy weight and fire at the full ratio, so three unfixable resources out of fifteen is a floor of ~5 points that no amount of work removes. Exclude them from those checks, keep them **in** the *Empty collection / category* check (a utility set that renders and is empty is still worth naming), and list the excluded titles under *Informational* so the exclusion is visible and the denominator is checkable. Per-check populations differ as a result — that is expected, and each check divides by its own.
...
Weights: no price in market 20, orphan variants 20, unbuyable in market 20, missing SKU 15, volume not set 15, missing images 15, duplicate SKUs 15, inconsistent currencies 15, missing description 10, weak SEO 10, boilerplate SEO 10, no category 10, broken images 10.

No price in market, orphan variants and unbuyable in market lead at 20 because they are the only three that make a product genuinely unsellable rather than merely untidy. *Volume not set* sits at 15 alongside *missing SKU* for the same reason those two belong together: the product sells fine and the damage lands after the sale — a missing SKU breaks fulfilment, missing `cv`/`qv` breaks the commission. Both are invisible at checkout and expensive afterwards.
...
The **Ready to sell** line is a blunt yes/no above the scores, because 88/100 reads like a pass while a product sits in the catalog that nobody can buy. It is NO whenever a product is unsellable for a **configuration** reason — no `variant_countries` row for the market, that row inactive, a null/zero price, or every variant `buyable: false` — or whenever the company has enrollment packs but **not one of them is published**, because a join flow whose every tier is draft or scheduled cannot take a recruit today. That last one is a population-level fact read off the pack list, not a per-pack deduction: the packs themselves stay out of the score. The count is of products, not issues.

Otherwise YES — and **the qualifier has to name the worst thing it is waving through, not label it.** "With cosmetic gaps" is only honest when the remaining gaps *are* cosmetic: missing copy, a thin description, a weak meta tag. When the catalog transacts but something downstream of the sale is broken — variants with no SKU that a warehouse cannot pick or reconcile, a declared bundle with no component groups — say that instead: `YES — 5 variants have no SKU and can't be fulfilled`. The line's question is "can this catalog transact?", so a fulfillment break does not flip it to NO, but it must not disappear behind a word that means "trivial" either. If you cannot fit the worst remaining defect into the qualifier, it belonged in the NO list.
...
- **Volume not set** — `cv` / `qv` live on each `variant_countries` row, so an importer that carried prices but not volume leaves a product that sells and pays nobody. Say which field is missing and that the rest of the catalog has it, because that contrast is the evidence. Pair it with the wholesale blind spot in the same breath when both apply: the volume gap is proven, the wholesale one is unverified, and the user should check both in the same pass through the admin.
...
- **Duplicate title or slug** — name the pair and which kind each is; usually an import created a category and a collection from the same source taxonomy. Describe the records, not the rendered site: two entry points into the same range, one in each admin screen.
...
Close with a single next step, not a menu: the highest-weight issue, phrased as an action. If the company has other open markets, add one line offering to run the same audit for them — and when the market you audited came back clean on prices and currencies, say so in that line, so the offer reads as "the same shared findings again" rather than hinting at something undiscovered.
...
- Don't use `/api/products` (legacy). It answers 200 but embeds the entire country configuration blob per product and will exhaust context on the second page. Note this is a *different* endpoint from `/api/company/v1/products`, which the membership shortcut above deliberately recommends — the ban is on the customer-facing one only.
