...
1. Call `fluid_api("/api/users/v2025-06/media?limit=100", "GET")`, paginating fully. Filter to `active: true`. Capture `id`, `kind` (video/pdf/image), `label`/`description`, `leads`, `attached_shareables`, `created_at`.
...
5. Also call `fluid_api("/api/enrollment_packs?limit=20", "GET")` (unversioned endpoint) and `fluid_api("/api/v2025-06/enrollment_packs/<id>/share_stats", "GET")` per pack — some direct-selling companies drive most of their sharing through enrollment packs rather than media, and skipping this would miss it.
