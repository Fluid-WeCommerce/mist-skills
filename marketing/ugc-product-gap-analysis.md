...
2. Pull the media library: `fluid_api("/api/users/v2025-06/media?limit=100", "GET")`, paginating fully, filtered to `active: true` and `kind` in `video`/`image`. Build a lowercased keyword index from each item's `title` (strip common filler words) — `label` is always `null` in practice, don't index it, and `description` (when present) is an object (`{ body: "<p>…</p>", … }`), not a plain string, so strip the HTML tags out of `description.body` before indexing it rather than treating the field itself as text.
