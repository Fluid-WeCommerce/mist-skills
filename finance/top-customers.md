...
1. Call `fluid_api("/api/v2025-06/customers?sort=-total_spent_cents&limit=20", "GET")` to fetch the top 20 by spend.
2. For each customer, optionally call `fluid_api("/api/v202506/customers/<id>/orders?limit=1&sort=-created_at", "GET")` to grab the most recent order date.
